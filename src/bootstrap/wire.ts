...
import { CommandLogRepository } from "../features/logs/command-log-repo.ts";
...
export async function createAppContext(): Promise<AppContext> {
  ...
  const googleCloudTts = config.bot.googleCloudApiKey
    ...
    : null;
  const userPrefs = new UserPrefsRepository({ db });
  const guildSettings = new GuildSettingsRepository({
    db,
    redis: redis ?? createNoopRedis(logger),
  });
  ...
  const commandLogRepo = new CommandLogRepository({ db });
  const commandLogger = new CommandLoggerService({ db, logger });
  const ttsCache = new TtsCacheService({ logger });
  const voiceCatalog = new WavenetVoiceCatalog({
    client: googleCloudTts,
    logger,
  });
  const basicProvider = new BasicTtsProvider({ logger });
...
function createNoopRedis(logger: Logger): RedisClient {
  return new RedisClient({ url: "redis://disabled", logger });
}

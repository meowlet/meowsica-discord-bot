...
function getShardCount(key: string, defaultValue: number | "auto"): number | "auto" {
  const value = Bun.env[key];
  ...
  if (isNaN(parsed) || parsed < 1) {
    throw new Error(`Environment variable ${key} must be a positive number or 'auto', got: ${value}`);
  }
  return parsed;
}
...
export function loadConfig(): BotConfig {
  ...
  return {
    ...
    testingGuildId: getEnvStringOrNull("TESTING_GUILD_ID") || getEnvStringOrNull("DISCORD_GUILD_ID"),
  };
}

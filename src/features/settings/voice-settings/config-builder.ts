...
import { SUPPORTED_LANGUAGES } from "../../tts/languages.ts";
...
export async function buildVoiceConfigInterface(
  ...
  const langCode = prefs.tts.language ?? "vi-VN";
  const supportsWavenet = await voiceCatalog.hasWavenetSupport(langCode);
...
async function buildVariantSelect(
  ...
  voiceCatalog: WavenetVoiceCatalog,
  locale: Locale,
): Promise<ActionRowBuilder<StringSelectMenuBuilder>> {
  const langCode = language ?? "vi-VN";
  const voices = await voiceCatalog.getVoicesForLanguage(langCode);

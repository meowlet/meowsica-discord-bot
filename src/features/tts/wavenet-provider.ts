...
export class WavenetTtsProvider {
  ...
  async synthesize(
    ...
    await this.usage.assertAndReserve(params.userId, params.text.length);
    let buffer: Buffer | null = null;
    try {
      buffer = await this.client.synthesize({
        text: params.text,
        voiceName,
        languageCode,
      ...
      await this.releaseQuiet(params.userId, params.text.length);
      throw err;
    }
    if (!buffer) {
      await this.releaseQuiet(params.userId, params.text.length);
  ...
  private async releaseQuiet(userId: string, chars: number): Promise<void> {
    try {
      await this.usage.releaseReservation(userId, chars);
    } catch (err) {
      this.logger.warn("failed to release quota reservation", err);
    }
  }
}

export default {
  common: {
    error: "An error occurred",
    success: "Success",
  },

  commands: {
    ping: {
      name: "ping",
      description: "Replies with Pong and shows latency",
      pinging: "Pinging...",
      title: "Pong!",
      subtitle: "Your latency and API latency are shown below.",
      latency: "Latency",
      apiLatency: "API Latency",
    },
    help: {
      name: "help",
      description: "Shows all available commands",
      title: "Available Commands",
      subtitle: "Here are all the commands you can use:",
      categories: {
        voice: "Voice Commands",
        tts: "TTS Commands",
        settings: "Settings",
        misc: "Utility",
      },
    },
    lang: {
      name: "lang",
      description: "Set bot language",
      option: "Choose a language",
      noPermission:
        "You need the Manage Server permission to change server settings.",
      serverOnly: "This command can only be used in a server.",
      interface: {
        description: "Set UI language",
        user: "Set your personal UI language",
        server: "Set UI language for this server",
        userSuccess: "Your UI language has been set to English!",
        serverSuccess: "Server UI language has been set to English!",
      },
      speech: {
        description: "Set speech/TTS language",
        user: "Set your personal speech language",
        server: "Set speech language for this server",
        userSuccess: "Your speech language has been set to English!",
        serverSuccess: "Server speech language has been set to English!",
      },
    },
    join: {
      description: "Join your voice channel",
      serverOnly: "This command can only be used in a server.",
      notInVoice: "You must be in a voice channel to use this command.",
      success: "Joined Voice Channel",
      joinedChannel: "Connected to **{channel}**",
      failed: "Failed to join the voice channel. Please try again.",
    },
    leave: {
      description: "Leave the voice channel",
      serverOnly: "This command can only be used in a server.",
      notConnected: "I'm not connected to any voice channel.",
      notInSameChannel:
        "You must be in the same voice channel as the bot to use this command.",
      success: "Left Voice Channel",
      disconnected: "Disconnected from the voice channel.",
      failed: "Failed to leave the voice channel.",
    },
    say: {
      description: "Say something using text-to-speech",
      messageOption: "The message to speak",
      langOption: "Language for TTS (overrides your default)",
      serverOnly: "This command can only be used in a server.",
      notInVoice: "You must be in a voice channel to use this command.",
      notInSameChannel: "You must be in the same voice channel as the bot.",
      emptyMessage: "Please provide a message to say.",
      messageTooLong: "Message is too long. Maximum {max} characters allowed.",
      invalidLanguage:
        "Invalid language code. Use `/voices` to see available languages.",
      success: "Speaking",
      speaking: 'Now saying: "{message}"',
      queued: 'Added to queue: "{message}"',
      failed: "Failed to play TTS audio.",
      joinFailed: "Failed to join your voice channel.",
    },
    stop: {
      description: "Stop TTS and leave the voice channel",
      serverOnly: "This command can only be used in a server.",
      notConnected: "I'm not connected to any voice channel.",
      notInSameChannel: "You must be in the same voice channel as the bot.",
      success: "Stopped",
      stopped: "Stopped playback and left the voice channel.",
      cleared: "Cleared {count} message(s) from the queue.",
    },
    skip: {
      description: "Skip the current TTS message",
      serverOnly: "This command can only be used in a server.",
      notConnected: "I'm not connected to any voice channel.",
      notInSameChannel: "You must be in the same voice channel as the bot.",
      nothingPlaying: "Nothing is currently playing.",
      success: "Skipped",
      skipped: "Skipped the current message.",
    },
    voices: {
      description: "Manage TTS voice settings",
      title: "Available TTS Languages",
      subtitle:
        "Use `/say lang:CODE` to speak in a specific language, or set your default with `/voices set`.",
      pageInfo: "Page {current} of {total}",
      totalLanguages: "{count} languages available",
      usage: "**Usage:** `/say message:Hello lang:ja` to speak in Japanese",
      list: {
        description: "List all available TTS voice languages",
        pageOption: "Page number",
      },
      set: {
        description: "Set your preferred TTS voice",
        languageOption: "Select a language",
        variantOption: "Select a Wavenet voice (Encore only)",
        providerOption: "Choose Basic (Free) or Encore (Wavenet)",
        success: "Voice Settings Updated",
        language: "Language",
        provider: "Provider",
        voice: "Model",
        encoreRequired: "Encore Required",
        encoreRequiredDesc:
          "This feature requires Meowsica Encore. Use Basic (Google Translate) for free access.",
        invalidLanguage: "Invalid language code. Please select from the autocomplete list.",
      },
      info: {
        description: "View your current voice settings",
        title: "Your Voice Settings",
        status: "Status",
        encoreActive: "Encore Active",
        standard: "Free Tier",
        language: "Language",
        provider: "Provider",
        model: "Model",
        upgradeHint: "Upgrade to Meowsica Encore for Wavenet voices!",
      },
      reset: {
        description: "Reset your voice settings to default",
        success: "Voice Settings Reset",
        successDesc: "Your voice settings have been reset to default (Basic English).",
      },
    },
    queue: {
      description: "Show the current TTS queue",
      serverOnly: "This command can only be used in a server.",
      notConnected: "I'm not connected to any voice channel.",
      title: "TTS Queue",
      empty: "The queue is empty.",
      nowPlaying: "Now Playing",
      upNext: "Up Next",
      moreItems: "...and {count} more",
      totalItems: "{count} message(s) in queue",
    },
    encoreAdmin: {
      description: "Manage Meowsica Encore subscriptions",
      noPermission: "You don't have permission to use this command.",
      grant: {
        description: "Grant Encore to a user",
        userOption: "The user to grant Encore to",
        daysOption: "Number of days (leave empty for lifetime)",
        success: "Encore Granted",
        grantedDays: "Granted **{user}** Meowsica Encore for **{days} days**!",
        grantedLifetime: "Granted **{user}** lifetime Meowsica Encore!",
      },
      revoke: {
        description: "Revoke Encore from a user",
        userOption: "The user to revoke Encore from",
        success: "Encore Revoked",
        revoked: "Revoked Meowsica Encore from **{user}**.",
      },
      check: {
        description: "Check a user's Encore status",
        userOption: "The user to check",
        title: "Encore Status",
        user: "User",
        status: "Status",
        active: "Active",
        inactive: "Inactive",
        expired: "Expired",
        expiresAt: "Expires",
        lifetime: "Lifetime",
      },
    },
    voice: {
      description: "Open your voice settings dashboard",
      dashboard: {
        title: "Voice Settings",
        subtitle: "Your personal TTS configuration dashboard",
        language: "Language",
        provider: "Provider",
        model: "Voice Model",
        status: "Status",
        providerBasic: "Basic (Google Translate)",
        providerEncore: "Encore (Google Wavenet)",
        modelDefault: "Default",
        statusFree: "Free Plan",
        statusActive: "Encore Active",
        statusLifetime: "Encore (Lifetime)",
        upgradeHint: "Upgrade to Meowsica Encore for Wavenet voices!",
      },
      buttons: {
        config: "Configure",
        reset: "Reset",
      },
      config: {
        title: "Voice Configuration",
        subtitle: "Customize your TTS voice settings below. Changes are saved automatically.",
        languagePlaceholder: "Select a language...",
        languageUpdated: "Language updated to {language}",
        moreLanguages: "More Languages...",
        moreLanguagesDesc: "View additional language options",
        backToTop: "Back to Top",
        backToTopDesc: "Return to main language list",
        providerPlaceholder: "Select TTS provider...",
        providerLocked: "Provider: Basic",
        providerBasicOnly: "Provider: Basic (Standard)",
        providerBasicLabel: "Basic (Google Translate)",
        providerBasicDesc: "Free TTS using Google Translate",
        providerEncoreLabel: "Encore (Google Cloud)",
        providerEncoreDesc: "High-quality Wavenet voices",
        providerPremiumLabel: "Encore (Google Cloud)",
        providerUpdated: "Provider updated to {provider}",
        encoreRequired: "This feature requires Meowsica Encore. Upgrade to unlock Wavenet voices!",
        variantPlaceholder: "Select voice variant...",
        variantLockedPlaceholder: "Not available in Basic mode",
        variantNotAvailable: "Not Available",
        variantRequiresEncore: "Upgrade to Encore for Wavenet voices",
        variantBasicMode: "Switch to Encore provider to select variants",
        variantNoVoices: "No voices available for this language",
        variantUpdated: "Voice updated to {variant}",
        noWavenetForLanguage: "Encore voices are not available for this language.",
        // Speed settings
        speedPlaceholder: "Select speech speed...",
        speedUpdated: "Speed updated to {speed}",
        speedNormal: "Normal",
        speedNormalDesc: "Default speech rate",
        speedSlow: "Slow Mode",
        speedSlowDesc: "Slower, clearer pronunciation",
        speed025x: "0.25x (Very Slow)",
        speed05x: "0.5x (Slow)",
        speed075x: "0.75x",
        speed10x: "1.0x (Normal)",
        speed125x: "1.25x",
        speed15x: "1.5x",
        speed20x: "2.0x (Fast)",
        speed30x: "3.0x",
        speed40x: "4.0x (Very Fast)",
        // Pitch settings (Encore only)
        pitchPlaceholder: "Select voice pitch...",
        pitchUpdated: "Pitch updated to {pitch}",
        pitchDeep: "Deep / Low",
        pitchDeepDesc: "Lower voice pitch",
        pitchMediumLow: "Medium Low",
        pitchMediumLowDesc: "Slightly lower pitch",
        pitchNormal: "Normal",
        pitchNormalDesc: "Default voice pitch",
        pitchMediumHigh: "Medium High",
        pitchMediumHighDesc: "Slightly higher pitch",
        pitchHigh: "High",
        pitchHighDesc: "Higher voice pitch",
      },
      reset: {
        success: "Settings Reset",
        successDesc: "Your voice settings have been reset to default (Basic Vietnamese).",
        notOwner: "Only the dashboard owner can reset these settings.",
      },
    },
    language: {
      description: "Manage interface language settings",
      dashboard: {
        title: "Language Settings",
        subtitle: "Manage your interface language preferences",
        userLanguage: "Your Language",
        serverLanguage: "Server Language",
        notSet: "Not set (using default)",
        priorityNote: "Your personal setting always overrides the server default.",
      },
      buttons: {
        configure: "Configure",
        close: "Close",
      },
      config: {
        title: "Language Configuration",
        subtitle: "Choose your preferred interface language. Changes are saved automatically.",
        userPlaceholder: "Select your language...",
        serverPlaceholder: "Select server language (Admin)...",
        userUpdated: "Your language has been set to {language}!",
        serverUpdated: "Server language has been set to {language}!",
        noPermission: "You need the Manage Server permission to change server settings.",
        serverOnly: "Server settings can only be changed in a server.",
      },
    },
    profile: {
      description: "View your TTS profile",
      title: "User Profile",
      status: {
        encore: "Encore Active",
        free: "Free Plan",
      },
      fields: {
        status: "Status",
        provider: "Provider",
        model: "Current Model",
        expires: "Expiration",
      },
      model: {
        auto: "Auto (Standard)",
      },
      expires: {
        lifetime: "Lifetime",
      },
    },
  },

  encore: {
    badge: "Meowsica Encore",
    modeActive: "Using Wavenet voice",
    expired: "Your Encore subscription has expired.",
    notAvailable: "Wavenet voices are currently unavailable.",
  },
};
